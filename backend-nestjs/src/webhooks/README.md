...
- **ViettelPost — không khớp đơn → tạo mới**: Khớp đơn theo trackingCode / metadata `$.partner.trackingCode` / ORDER_REFERENCE / `PCK-{id}`. Nếu không tìm thấy → TẠO ĐƠN MỚI: `source='VIETTEL'`, userId=null (khách vãng lai), orderCode=ORDER_NUMBER, totalAmount=MONEY_COLLECTION (COD), status map từ ORDER_STATUS, trackingCode lưu vào metadata. Các trạng thái cuối: 101→PENDING, 107→CONFIRMED, 201→SHIPPING, 501→DELIVERED, 503/504→CANCELLED.
...
- **Voucher VTP**: giao thành công (501/515) → hẹn mở voucher sau 7 ngày (BullMQ) hoặc mở ngay nếu queue tắt; thất bại (502–504) → reject. Giao 1 phần → tính lại discount theo tỷ lệ.
