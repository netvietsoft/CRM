...
// Đơn hàng CCM — nguồn đơn RIÊNG (source='CCM'): đơn chốt từ hội thoại CCM (Messenger/Zalo…),
// gồm đơn AI chốt + nhân viên tạo từ chat + tạo tay từ nút Tạo đơn của trang này.
// Dùng chung toàn bộ UI với /admin/orders (bảng, lọc, chi tiết, đẩy VTP) — chỉ khoá filter nguồn.
import Link from 'next/link';
import { getSession } from '@/lib/auth';
import OrdersTableClient from '@/components/admin/OrdersTableClient';
import { apiClient } from '@/lib/apiClient';

interface OrderUserSummary { name?: string | null; phone?: string | null }
interface OrderMetadataItem { name?: string | null; quantity?: number | null }
interface OrderMetadata { items?: OrderMetadataItem[] | null }
interface OrderItemSummary { quantity: number; product?: { name?: string | null } | null }
...
export default async function CcmOrdersPage(props: {
  ...
  try {
    ...
    console.error('Error fetching CCM orders:', error);
  }
  ...
  const buildPageUrl = (page: number) => {
    ...
    return `/admin/ccm-orders?${p.toString()}`;
  };
  ...
  return (
    <>
      <OrdersTableClient
        ...
        title="Đơn hàng CCM"
        subtitle="Đơn chốt từ hội thoại CCM (Messenger/Zalo…) — tạo đơn, sửa đơn, đẩy vận chuyển"
        createSource="CCM"
        showRevenue={false}
      />
      ...
      {pagination.totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 mt-8 mb-4">
          {pagination.page > 1 && (
            <Link href={buildPageUrl(pagination.page - 1)} className="px-3 py-2 rounded-lg text-sm font-medium bg-white text-gray-700 border border-gray-300 hover:bg-gray-50 transition-colors">←</Link>
          )}
