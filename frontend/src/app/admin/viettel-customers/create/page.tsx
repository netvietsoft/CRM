  ...
  if (result?.trackingCode) {
    return (
      <div className="max-w-lg mx-auto mt-10 bg-white rounded-2xl border border-gray-100 shadow-sm p-8 text-center space-y-4">
        ...
        <h1 className="text-xl font-bold text-gray-900">{cancelled ? 'Đã hủy đơn ViettelPost' : 'Tạo đơn ViettelPost thành công'}</h1>
        ...
        <div className="flex gap-3 justify-center flex-wrap pt-2">
          <button onClick={resetForm} className="px-4 py-2 rounded-xl bg-gray-100 hover:bg-gray-200 text-sm">Tạo đơn khác</button>
          {!cancelled && (
            <button onClick={() => void cancelOnVtp()} disabled={cancelling} className="px-4 py-2 rounded-xl bg-red-600 hover:bg-red-700 text-white text-sm font-semibold disabled:opacity-50">
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-gray-900">➕ {draftCode ? 'Sửa nháp đơn ViettelPost' : 'Tạo đơn ViettelPost'}</h1>
        {draftCode && <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-amber-100 text-amber-700">Nháp</span>}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 items-start">
        ...
        <div className="space-y-5">
          ...
          <div className={card}>
            <div className="flex items-center justify-between">
              ...
              <label className="flex items-center gap-2 cursor-pointer text-xs text-gray-600 select-none">
                ...
                <button type="button" role="switch" aria-checked={useNewAddress} onClick={toggleNewAddress}
                  className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${useNewAddress ? 'bg-indigo-600' : 'bg-gray-300'}`}>
            ...
            {useNewAddress && <p className="text-xs text-amber-600 -mt-2">Hệ địa giới mới (sau 1/7/2025): chỉ chọn Tỉnh → Phường/Xã.</p>}
            <div className="grid grid-cols-2 gap-3">
            ...
            <div className={`grid gap-3 ${useNewAddress ? 'grid-cols-2' : 'grid-cols-3'}`}>
              <div>
        ...
        <div className="space-y-5">
          ...
          <div className={card}>
            ...
            <div><label className={lbl}>Mã đơn hàng (tự sinh: CHYSHOP + số đơn, sửa được)</label><input className={inp} value={orderReference} onChange={e => setOrderReference(e.target.value)} /></div>
          </div>
          ...
          <div className={card}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className={lbl}>COD (tiền thu hộ, đ) — mặc định = tổng giá trị</label><input className={inp} type="number" value={cod} onChange={e => { setCod(e.target.value); setCodTouched(true); }} /></div>
              <div>
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={() => setOrderNote(DEFAULT_NOTE)} className="text-xs text-indigo-600 hover:text-indigo-800">↺ Ghi chú mặc định</button>
              </div>
              <textarea rows={3} className={inp} value={orderNote} onChange={e => setOrderNote(e.target.value)} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <button onClick={() => void create()} disabled={creating || savingDraft} className="lg:px-10 px-4 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold disabled:opacity-50">
          {creating ? 'Đang tạo đơn trên ViettelPost...' : '🚀 Tạo đơn & đẩy sang ViettelPost'}
        </button>
        <button onClick={() => void saveDraft()} disabled={savingDraft || creating} className="px-6 py-3 rounded-xl bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-semibold disabled:opacity-50">
          {savingDraft ? 'Đang lưu...' : '💾 Lưu nháp'}
        </button>
        <button onClick={() => void discardOrDeleteDraft()} disabled={creating || savingDraft} className="px-6 py-3 rounded-xl bg-red-50 hover:bg-red-100 text-red-600 font-semibold disabled:opacity-50">
