...
export default function CcmTopNav({ userName = 'CHY' }: { userName?: string }) {
  const pathname = usePathname();
  return (
    <header className="h-14 bg-[#3b5bdb] text-white flex items-center px-4 gap-6 shrink-0">
      <Link href="/ccm/conversations" className="flex items-center gap-2 font-semibold text-lg">
        <span className="w-7 h-7 rounded-full bg-white/20 grid place-items-center">◐</span> Ccm
      </Link>
