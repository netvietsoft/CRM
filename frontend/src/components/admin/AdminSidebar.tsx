...
export default function AdminSidebar({ user, isOpen = true, unreadCount = 0, pendingStoresCount = 0, onClose }: AdminSidebarProps) {
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-50 md:relative bg-[#f0f2f5] pt-6 text-black w-64 flex-shrink-0 overflow-y-auto transition-transform duration-300 border-r border-gray-200 ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0 md:w-0 md:overflow-hidden md:border-none'}`}>
